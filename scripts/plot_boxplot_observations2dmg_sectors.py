...
import dask
def load_nc_obs_data( path2data, region_ID, roi_type='sector', varname=None ):
    ''' ----------------------
    ...
    if not varname: # load all available variables
        ...
        region_varfile = glob.glob(path2data + '*sector-'+region_ID+ '*'+ varname +'*nc')[0]
...
def stack_ds_to_df(region_ds, iceshelf_polygon_gpd = iceshelf_poly_meas):
    ...
    - Clip 2D to ice shelf (2D xarray)
    ...
    ''' Clip to iceshelf  '''
    # iceshelf_polygon_gpd = iceshelf_poly_meas # OK to clip with static ice shelf polygon, I use "REMA" "VELOCITY" and "BASALMELT" that only have static mask anyway
...
Load data: netCDFs per region, per variable; and plot boxplot 
...
def main( region_ID, sector_ID, dmg_type = 'dmg095', ksize=None, process_all_AIS=False):
    ...
    if not process_all_AIS:
        region_ds = load_nc_obs_data( path2data, region_ID, roi_type='sector', varname=None )
        ''' ------------------------------
        ## DATA (PRE)PROCESSING
    ...
    ''' ------------------------------
    ## Plot box/bar plot and save 
    ----------------------------------'''
    print('.. plotting')
    
    # set min-max ranges of plots
